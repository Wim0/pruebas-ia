...
let ClerkAuthGuard = class ClerkAuthGuard {
    ...
    async canActivate(context) {
        ...
        try {
            const payload = await clerk_sdk_node_1.clerkClient.sessions.verifySession(sessionToken, '');
            if (!payload) {
                throw new common_1.UnauthorizedException('Invalid session.');
            }
            const user = await clerk_sdk_node_1.clerkClient.users.getUser(payload.userId);
            request.user = user;
            ...
            if (!hasRequiredRole) {
                throw new common_1.ForbiddenException('You do not have the required role(s).');
            }
            return true;
        }
        catch (error) {
